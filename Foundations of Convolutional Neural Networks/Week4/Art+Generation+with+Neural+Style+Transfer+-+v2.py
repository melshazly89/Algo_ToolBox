...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.reshape(tf.transpose(a_C,perm=None),(n_H*n_W,n_C))
    a_G_unrolled = tf.reshape(tf.transpose(a_G,perm=None),(n_H*n_W,n_C))
    #print("--1%d\n",a_C_unrolled.shape)
    #print("--2%d\n",a_G_unrolled.shape)
...
def gram_matrix(A):
    ...
    GA = tf.matmul (a=A,b=A,transpose_a=False,transpose_b=True)
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.transpose(tf.reshape(a_S,(n_H*n_W,n_C)),perm=None)
    a_G = tf.transpose(tf.reshape(a_G,(n_H*n_W,n_C)),perm=None)
    ...
    J_style_layer = (1/(4*((n_C*n_C))*((n_W*n_H*n_W*n_H))))*tf.reduce_sum(tf.square(tf.subtract(GS,GG)))
